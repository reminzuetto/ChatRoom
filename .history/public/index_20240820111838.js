...
document.addEventListener("DOMContentLoaded", function () { // Lấy thông tin từ localStorage const name = localStorage.getItem("name"); const room = localStorage.getItem("room"); // Kiểm tra nếu đã có tên và phòng thì hiển thị chúng if (name && room) { // Cập nhật phần tử hiển thị tên người dùng và phòng const roomDisplay = document.getElementsByClassName("dropdown-button"); roomDisplay[0].textContent = `Room: ${room}`; } });
